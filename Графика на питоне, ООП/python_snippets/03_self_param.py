...
my_son_backpack = Backpack() #мы присваеваем переменной поведенческую схему класса Backpack
my_son_backpack.add(item='учебник') #пользуемся методами класса, для работы с переменными

# то есть аналогия такая, что были вызовы типа таких
# add(self=my_backpack, item='ноутбук') #мы можем работать с переменными на прямую через сам метод
