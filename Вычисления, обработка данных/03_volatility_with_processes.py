...
class CalcVolatility(Process):

    def __init__(self, file_name, collector, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # self.volatilities = defaultdict(float)
    ...
    def run(self):
        ...
        self.collector.put(dict(secid=self.secid, volatility=self.volatility))
...
def get_vols(main_file_name):
    ...
    tickers = [CalcVolatility(file_name=full_name, collector=collector) for full_name in full_files_names]
    ...
    while not collector.empty():
        data = collector.get()
        volatilities[data['secid']] = data['volatility']
